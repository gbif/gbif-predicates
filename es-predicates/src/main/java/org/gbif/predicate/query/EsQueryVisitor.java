...
public abstract class EsQueryVisitor<S extends SearchParameter> implements QueryVisitor {
  ...
  private static RangeRelation parseRangeRelation(String relation) {
    if (relation == null) return null;
    return RangeRelation.valueOf(
        Character.toUpperCase(relation.charAt(0)) + relation.substring(1).toLowerCase());
  }

  private static Query rangeQuery(
      String field, Object gte, Object lte, Object gt, Object lt, String relation) {
    RangeRelation rangeRelation = parseRangeRelation(relation);
    return Query.of(
        q ->
            q.range(
                r ->
                    r.untyped(
                        u -> {
                          ...
                          if (rangeRelation != null) u.relation(rangeRelation);
                          return u;
                        })));
  }
    ...
    for (Map.Entry<S, List<SimplePredicate<S>>> entry : rangeBoundsByKey.entrySet()) {
      ...
      Query q =
          addNullableFieldPredicate(
              bounds.get(0),
              rangeQuery(esFieldMapper.getExactMatchFieldName(key), gte, lte, gt, lt, null));
      ...
      try {
        ...
        if (mustQueryData.isNested()) {
          ...
          if (subPredicate instanceof DisjunctionPredicate && mustQueryData.rawQueries.size() > 1) {
            Query shouldWrapper =
                buildBoolQuery(
                    Collections.emptyList(), mustQueryData.rawQueries, Collections.emptyList());
  ...
  public void visit(InPredicate<S> predicate, QueryData queryData) {
    ...
    } else if (paramEquals(parameter, OccurrenceSearchParameter.GEOLOGICAL_TIME.name())) {
  ...
  public void visit(NotPredicate predicate, QueryData queryData) throws QueryBuildingException {
    ...
    QueryData existsQueryData = new QueryData();
    addNotPredicateNonNullFilters(predicate.getPredicate(), existsQueryData);
    queryData.filterQueries.addAll(existsQueryData.filterQueries);
  ...
  private void addNotPredicateNonNullFilters(Predicate predicate, QueryData queryData) {
    ...
    fieldsByParameter.forEach(
        (parameter, fields) ->
            fields.forEach(field -> addFilterQuery(existsQuery(field), queryData, parameter)));
  }
